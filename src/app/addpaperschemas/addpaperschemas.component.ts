    ...
    @Component({
      selector: 'app-addpaperschemas',
      templateUrl: './addpaperschemas.component.html',
      styleUrls: ['./addpaperschemas.component.css']
    })
    export class AddpaperschemasComponent implements OnInit, AfterViewInit {
      ...
      onSubmit() {
        ...
        switch (true) {
          ...
          default:
            try {
              this.startLoading()
              this.dbservice.post('schemas',formValue).subscribe((res)=>{
                console.log(res);
                if (res) {
                  this.msg.success('Sehema created successfully')
                  this.form.reset()
                  this.closeLoading()
                }
              })
              ...
              console.log(error);
              this.msg.error('Some error founded try again')
              this.closeLoading()
            }
        }
      }
      ...
      startLoading() {
        this.loader = true
        document.getElementById('loaderdiv2')?.classList.add('disabled')
      }
      closeLoading() {
        this.loader = false
        document.getElementById('loaderdiv2')?.classList.remove('disabled')
      }
